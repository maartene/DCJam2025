...
@Suite("All abilities should") struct AbilityTests {
    ...
    @Suite("be able to be combined") struct CombineAbilities {
        ...
        @Test("into an ability where similar properties are added together")
        func addTogetherSimilarProperties() throws {
        ...
        @Test("and have a key that is a combination of the combined ability keys")
...
@Suite("Concrete abilities should") struct ConcreteAbilityTests {
    @Suite("for healing abilities") struct SingleAbilities {
        ...
        @Test("heal more health when an ability is combined with a heal boost")
        func healMoreHealthWhenCombinedWithAHealBoost() throws {
    ...
    @Suite("For offensive abilities") struct OffensiveAbilities {
        ...
        @Test("deal damage to a target") func dealDamageToATarget() throws {
            ...
            let target = try #require(
                world.enemiesOnCurrentFloor.first {
        ...
        @Test("deal damage in AoE") func dealDamageInAoE() throws {
            ...
            for enemyIndex in 0..<enemies.count {
                #expect(enemies[enemyIndex].currentHP < hpBeforeAbility[enemyIndex])
            }
        }

        @Test("not deal damage with an AoE if there is no straight LoS impact position")
        func doesNotDealAoEDamageWhenThereIsLoSImpact() throws {
            let world = makeWorld(from: [
                """
                ...
                .p.
        ...
        @Test("deal damage in a range") func dealDamageInRange() throws {
            ...
            let enemy = try #require(
                world.enemiesOnCurrentFloor.first(where: { $0.position == Coordinate(x: 0, y: 2) }))
        ...
        @Test("not damage in a range when it is obstructed")
        func notDealDamageInRangeWhenObstructed() throws {
            let world = makeWorld(from: [
                """
                ...
                #..
        ...
        @Test("deal more damage when an ability is combined with a damage boost")
        func dealMoreDamageWhenCombinedWithADamageBoost() throws {
            let target = try #require(
                world.enemiesOnCurrentFloor.first(where: { $0.position == Coordinate(x: 1, y: 2) }))
            let caster = world.partyMembers[.frontLeft]
            let damageEnemyAbility = DamageEnemyAbility()
